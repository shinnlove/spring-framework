...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	...
	/** Set of registered singletons, containing the bean names in registration order. 注册单例的集合，按注册顺序包含所有的beanId */
	...
	/** Disposable bean instances: bean name to disposable instance. beanId=>可销毁的bean实例映射 */
	...
	public Object getSingleton(String beanName, ObjectFactory<?> singletonFactory) {
		...
		synchronized (this.singletonObjects) {
			...
			if (singletonObject == null) {
				...
				try {
					// 在这一步trylambda表达式中getObject对象的方法
					singletonObject = singletonFactory.getObject();
					newSingleton = true;
				}
	 ...
	 * 同时被依赖bean => 所有需要他的bean集合(dependenciesForBeanMap)。
	 *
	...
	/**
	 * 检测一个特定被依赖的bean是否已经被注册成为一个给定bean、或其祖先bean依赖。
	 *
	...
	private boolean isDependent(String beanName, String dependentBeanName, @Nullable Set<String> alreadySeen) {
		// 事先传入的集合不等于null并且包含bean(检索到自己了，说明自己不依赖于给定的bean，自己有可能是被检测bean的依赖bean)
		...
		for (String transitiveDependency : dependentBeans) {
			...
			// `transitiveDependency`是任意一个依赖的bean
			// 检测给定bean的所有依赖bean是否也依赖于最早给出的bean
